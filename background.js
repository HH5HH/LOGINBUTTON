...
const LOGINBUTTON_LATEST_MANIFEST_URL =
  `https://raw.githubusercontent.com/${LOGINBUTTON_GITHUB_OWNER}/${LOGINBUTTON_GITHUB_REPO}/main/manifest.json`;
const LOGINBUTTON_LATEST_MANIFEST_API_URL =
  `https://api.github.com/repos/${LOGINBUTTON_GITHUB_OWNER}/${LOGINBUTTON_GITHUB_REPO}/contents/manifest.json?ref=main`;
...
function buildLatestLoginButtonManifestRawUrl(ref = "") {
  const manifestRef = normalizeCommitSha(ref);
  return manifestRef
    ? `https://raw.githubusercontent.com/${LOGINBUTTON_GITHUB_OWNER}/${LOGINBUTTON_GITHUB_REPO}/${manifestRef}/manifest.json`
    : LOGINBUTTON_LATEST_MANIFEST_URL;
}

function buildLatestLoginButtonManifestApiUrl(ref = "") {
  const manifestRef = normalizeCommitSha(ref);
  return manifestRef
    ? `https://api.github.com/repos/${LOGINBUTTON_GITHUB_OWNER}/${LOGINBUTTON_GITHUB_REPO}/contents/manifest.json?ref=${manifestRef}`
    : LOGINBUTTON_LATEST_MANIFEST_API_URL;
}

async function fetchLatestLoginButtonVersionFromRaw(ref = "") {
  const response = await fetch(buildLatestLoginButtonManifestRawUrl(ref), { cache: "no-store" });
  if (!response.ok) {
...
async function fetchLatestLoginButtonVersionFromGithubApi(ref = "") {
  const response = await fetch(buildLatestLoginButtonManifestApiUrl(ref), { cache: "no-store" });
  if (!response.ok) {
    throw new Error(`HTTP ${response.status}`);
  }
...
async function openLoginButtonGetLatestFlow() {
  ...
  const downloadUrl = buildLatestLoginButtonPackageUrl(latestCommitSha);
  const downloadFileName = buildLatestLoginButtonPackageFileName(latestVersion, latestCommitSha);
  const result = {
    ok: false,
    downloadUrl,
    downloadFileName,
