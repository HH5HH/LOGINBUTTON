...
function loadGetLatestHelpers(seed = {}) {
  ...
  const script = [
    ...
    'const LOGINBUTTON_LATEST_MANIFEST_URL = `https://raw.githubusercontent.com/${LOGINBUTTON_GITHUB_OWNER}/${LOGINBUTTON_GITHUB_REPO}/main/manifest.json`;',
    'const LOGINBUTTON_LATEST_MANIFEST_API_URL = `https://api.github.com/repos/${LOGINBUTTON_GITHUB_OWNER}/${LOGINBUTTON_GITHUB_REPO}/contents/manifest.json?ref=main`;',
    ...
    extractFunctionSource(source, "buildLatestLoginButtonManifestRawUrl"),
    extractFunctionSource(source, "buildLatestLoginButtonManifestApiUrl"),
...
test("openLoginButtonGetLatestFlow uses a SHA-pinned loginbutton_distro.zip when GitHub ref lookup succeeds", async () => {
  ...
  const seed = createSeed({
    ...
    responseByUrl: {
      [`https://raw.githubusercontent.com/HH5HH/LOGINBUTTON/${latestSha}/manifest.json`]: {
        ok: true,
        status: 200,
        async json() {
      ...
      "https://raw.githubusercontent.com/HH5HH/LOGINBUTTON/main/manifest.json": {
        ok: true,
        status: 200,
        async json() {
          return { version: "9.9.9" };
...
test("openLoginButtonGetLatestFlow prefers GitHub manifest API version over stale raw main manifest cache", async () => {
  const latestSha = "89abcdef0123456789abcdef0123456789abcdef";
  const encodedManifest = Buffer.from(JSON.stringify({ version: "1.2.77" }), "utf8").toString("base64");
  const seed = createSeed({
    currentVersion: "1.2.74",
    responseByUrl: {
      "https://api.github.com/repos/HH5HH/LOGINBUTTON/contents/manifest.json?ref=89abcdef0123456789abcdef0123456789abcdef": {
        ok: true,
        status: 200,
        async json() {
          return { content: encodedManifest };
        }
      },
      "https://raw.githubusercontent.com/HH5HH/LOGINBUTTON/main/manifest.json": {
        ok: true,
  ...
  assert.equal(response.ok, true);
  assert.equal(String(response.latestVersion || ""), "1.2.77");
  assert.equal(String(response.latestCommitSha || ""), latestSha);
  assert.match(String(seed.calls.downloadsDownload[0]?.filename || ""), /^LoginButton-v1\.2\.77-89abcde\.zip$/);
});

test("openLoginButtonGetLatestFlow falls back to main ZIP URL with cache bust when SHA lookup fails", async () => {
  const seed = createSeed({ currentVersion: "1.2.30" });
  const helpers = loadGetLatestHelpers(seed);
  const response = await helpers.openLoginButtonGetLatestFlow();
  ...
  assert.match(String(seed.calls.downloadsDownload[0]?.filename || ""), /^LoginButton-vlatest\.zip$/);
  assert.equal(String(seed.calls.tabsCreate[0]?.url || ""), "chrome://extensions");
});
